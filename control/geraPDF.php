...
function buscarTodos($idconcert) {
    ...
    if ($connection != NULL):
        try {
            $processResult = $connection->query("SELECT name, cpf, telefone FROM orquestrasinfo.ingressos WHERE idconcert = $idconcert ORDER BY name;");
            if ($processResult->rowCount() > 0):
